...
export default function UserDrawer() {
    const { user, showUserMenu, closeUserMenu, logout, addAddress, removeAddress, addPayment, deactivateAccount } = useAuth();
    ...
    return (<>
        ...
        <div style={{
            ...
            {view === 'orders' && (<>
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: '24px' }}>
                    {user.orders?.length > 0 ? (<div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                        {user.orders.map(order => (<div key={order.id} style={{
                            background: 'rgba(255,255,255,0.03)',
                            padding: '16px',
                            border: '1px solid rgba(255,255,255,0.05)'
                        }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                                <span style={{ fontWeight: 600, color: '#f4f2ee' }}>{order.id}</span>
                                <span style={{
                                    fontSize: '0.75rem',
                                    padding: '2px 8px',
